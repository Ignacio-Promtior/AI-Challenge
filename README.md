...
A **Retrieval-Augmented Generation (RAG)** chatbot that answers questions about the [Promtior](https://www.promtior.ai) website, built with **LangChain + LangServe + Ollama (LLaMA 3.2 3B)**.
...
└─────────────────┘    └──────────────────┘    └─────────────────┘
                               ▲                        │
                       OllamaEmbeddings          Ollama LLaMA 3.2 3B
...
4. Pull the LLaMA 3.2 3B model (first run only — ~2 GB)
...
- **LLM:** LLaMA 3.2 3B via Ollama (local, no API key required). Configured in `chain.py` via `OLLAMA_MODEL`.
